# Phase 02: Scheduled Briefings Notebook Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `notebooks/02_scheduled_briefings_complete.ipynb` (+ `_todo` flavor): the scratch→long-term promotion path rebuilt with the review's Path-1 fixes — opt-in triage curation, whole-note promotion with queryable provenance and supersession, a genuinely scheduled DBMS_SCHEDULER producer with a queue-depth health check, and a scheduled morning asset-risk briefing, plus a continuity-of-work demonstration.

**Architecture:** Pure decision logic goes in `src/cityops_harness/promote.py` (offline-tested). The notebook (generated by `tools/make_02_notebook.py`) wires it to: a `HARNESS_SCRATCH` note store, the `oracleagentmemory` SDK (first live use, `CITY_` prefix, `extract_memories=False` — whole notes, no re-extraction), a `HARNESS_MEMORY_META` sidecar holding provenance + supersession as queryable SQL, an in-DB staging procedure + two DBMS_SCHEDULER jobs, and a JSON briefing the LLM narrates but never computes.

**Tech Stack:** Python 3.12, python-oracledb, oracleagentmemory SDK (`add_memory`/`search`/`update_memory`), PL/SQL + DBMS_SCHEDULER, LangChain structured outputs, nbformat.

## Global Constraints

- Working dir `/Users/anantsr/Documents/Projects/total_recall`, branch `foundation`, venv `.venv/`.
- **Never touch** `notebook_complete_ollama.ipynb` / `total_recall_complete (1).ipynb`; never stage `wallet/`, `.env`, `.venv/`; never DROP or modify tables outside `HARNESS_%` (CITY_% tables are shared; the SDK owns `CITY_MEMORY` etc. via `CREATE_IF_NECESSARY`).
- All pytest tests run without DB/LLM/network.
- Exact SDK signatures are in `.superpowers/sdd/phase01-api-reference.md` — notebook tasks must read it; `add_memory(content, *, memory_type=..., user_id=..., agent_id=..., metadata=...) -> str(id)`, `search(query, *, user_id=..., agent_id=..., max_results=..., record_types=...) -> list[SearchResult]` (`.id`, `.content`, `.record.record_type`), `update_memory(memory_id, content=..., metadata=...)`. `create_thread/get_thread` are sync; `add_memory/search` have sync + async twins — the notebook uses the **sync** forms (no threads needed in 02).
- New tables (idempotent create): `HARNESS_SCRATCH`, `HARNESS_PROMOTION_QUEUE`, `HARNESS_MEMORY_META`, `HARNESS_BRIEFING`. New PL/SQL: `HARNESS_STAGE_INBOX`, `HARNESS_BUILD_BRIEFING` procedures; jobs `HARNESS_STAGE_JOB` (hourly), `HARNESS_BRIEFING_JOB` (daily 06:00). Job/proc creation is idempotent via drop-first with swallowed exceptions **scoped to those exact names only**.
- DB privileges: `ANANT` needs `CREATE PROCEDURE` and `CREATE JOB` (grant may still be pending — the setup section checks `user_sys_privs` and fails with the exact GRANT statements if missing; the live-run task must confirm before executing).
- Scratch status codes: `N`=new, `S`=staged, `P`=promoted, `D`=discarded, `H`=hold. Opt-in promotion area: paths starting `/inbox/`. Note path convention `/inbox/<asset_id>/<n>.md`; working files under `/work/`, `/tool_out/`, `/plans/` are never promoted.
- Thresholds/defaults (exact): `queue_health(pending, oldest_age_minutes, warn_pending=10, stall_minutes=120)` → `"stalled"` if `pending > 0 and oldest_age_minutes >= stall_minutes`, else `"warn"` if `pending >= warn_pending`, else `"ok"`; `rerank_by_recency(rows, half_life_days=90.0, base_key="base", age_key="days_ago")` → score = `base * 0.5 ** (age/half_life)`, sorted descending, score written to `row["score"]`.
- Learner-gap convention unchanged (`# ✏️ TODO(n):` + `# TODO-SOLUTION-START/END`); exactly 5 TODOs in this notebook.
- Notebook files: `notebooks/02_scheduled_briefings_complete.ipynb` (generated, kernelspec `harness`) + `_todo` via `tools/make_student.py`.
- Commits end with `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`.

---

### Task 1: Promotion decision helpers (`promote.py`)

**Files:**
- Create: `src/cityops_harness/promote.py`
- Test: `tests/test_promote.py`

**Interfaces:**
- Produces: `note_revision(content: str) -> str` (sha256 hex, 64 chars); `triage_allowed(path: str, allowed_prefix: str = "/inbox/") -> bool`; `queue_health(pending, oldest_age_minutes, warn_pending=10, stall_minutes=120) -> str`; `rerank_by_recency(rows, half_life_days=90.0, base_key="base", age_key="days_ago") -> list[dict]`.
...
Run: `.venv/bin/pytest tests/test_promote.py -q`
Expected: FAIL with `ModuleNotFoundError: No module named 'cityops_harness.promote'`

- [ ] **Step 3: Implement `src/cityops_harness/promote.py`**
...
Run: `.venv/bin/pytest tests/test_promote.py -q`
Expected: 6 passed. Then `.venv/bin/pytest -q` → 57 passed.

- [ ] **Step 5: Commit**

```bash
git add src/cityops_harness/promote.py tests/test_promote.py
git commit -m "feat: promotion pipeline decision helpers
...
### Task 2: State registry entries for notebook 02
...
**Interfaces:**
- Produces: `REGISTRY["02_scheduled_briefings"]` with six checks; notebook 03 will call `require(conn, "02_scheduled_briefings")`.
...
Run: `.venv/bin/pytest tests/test_state.py::test_registry_seeds_notebook_02 -q`
Expected: FAIL with `KeyError: '02_scheduled_briefings'`

- [ ] **Step 3: Extend `REGISTRY` in `src/cityops_harness/state.py`**

Add after the `"01_self_improving_copilot"` entry:
...
git commit -m "feat: artifact checks for notebook 02
...
### Task 3: Notebook generator A — setup, SDK init, scratch store

**Files:**
- Create: `tools/make_02_notebook.py`

**Interfaces:**
- Consumes: `.superpowers/sdd/phase01-api-reference.md` (READ FIRST — SDK signatures + embedder bridge are taken from it).
- Produces: generator with `SECTIONS = [INTRO, SETUP, SDK_INIT, SCRATCH]`; cell-level names later tasks rely on: `settings, conn, embedder, CHAT, HANDLER, memory (OracleAgentMemory), write_note(path, content), read_note(path), list_notes(prefix="/"), set_note_status(path, status), ASSET_A, INBOX_SEED (list of (path, content, days_ago)), SUPERSEDE_DEFECT_PATH, SUPERSEDE_REPAIR_PATH`.

- [ ] **Step 1: Write `tools/make_02_notebook.py`**
...
Run:
```bash
.venv/bin/python tools/make_02_notebook.py && .venv/bin/python - <<'EOF'
import ast, json
nb = json.load(open('notebooks/02_scheduled_briefings_complete.ipynb'))
n = 0
for c in nb['cells']:
    if c['cell_type'] == 'code':
...
EOF
```
Expected: `compiled 6 code cells`.

- [ ] **Step 3: Commit**

```bash
git add tools/make_02_notebook.py notebooks/02_scheduled_briefings_complete.ipynb
git commit -m "feat: notebook 02 generator - setup, whole-note SDK init, scratch store
...
### Task 4: Notebook generator B — triage gate, promotion, supersession

**Files:**
- Modify: `tools/make_02_notebook.py` (insert sections; `SECTIONS = [INTRO, SETUP, SDK_INIT, SCRATCH, TRIAGE, PROMOTION]`)

**Interfaces:**
- Consumes: Task 3 cell names; `promote.triage_allowed`, `promote.note_revision`.
- Produces: `Triage` model; `triage_note(path, content) -> Triage`; `promote_note(path) -> str | None` (memory_id or None); sidecar table `HARNESS_MEMORY_META`; `asset_from_path(path) -> str`. Contains TODOs 1–3.

- [ ] **Step 1: Insert into `tools/make_02_notebook.py`** (before the `SECTIONS = ...` line; update that line to `SECTIONS = [INTRO, SETUP, SDK_INIT, SCRATCH, TRIAGE, PROMOTION]`)
...
Same command as Task 3 Step 2. Expected: `compiled 8 code cells`.

- [ ] **Step 3: Commit**

```bash
git add tools/make_02_notebook.py notebooks/02_scheduled_briefings_complete.ipynb
git commit -m "feat: notebook 02 - triage gate, whole-note promotion, supersession sidecar
...
### Task 5: Notebook generator C — scheduled pipeline and briefing automation

**Files:**
- Modify: `tools/make_02_notebook.py` (insert sections; `SECTIONS = [INTRO, SETUP, SDK_INIT, SCRATCH, TRIAGE, PROMOTION, PIPELINE, BRIEFING]`)

**Interfaces:**
- Consumes: Task 3–4 names (`promote_note`, `_table_exists`, `conn`, `CHAT`, `HANDLER`).
- Produces: `HARNESS_PROMOTION_QUEUE`, procedures + jobs (`HARNESS_STAGE_INBOX`/`HARNESS_STAGE_JOB`, `HARNESS_BUILD_BRIEFING`/`HARNESS_BRIEFING_JOB`), `drain_queue(limit=50) -> int`, `pipeline_health() -> str`, `HARNESS_BRIEFING` table, `latest_briefing() -> dict | None`, `narrate_briefing() -> str`. Contains TODOs 4–5.

- [ ] **Step 1: Insert into `tools/make_02_notebook.py`** (update `SECTIONS` accordingly)
...
Same command. Expected: `compiled 14 code cells`.

- [ ] **Step 3: Commit**

```bash
git add tools/make_02_notebook.py notebooks/02_scheduled_briefings_complete.ipynb
git commit -m "feat: notebook 02 - scheduled producer/consumer, health check, briefing automation
...
### Task 6: Notebook generator D — supersession demo, recall, continuity, closing

**Files:**
- Modify: `tools/make_02_notebook.py` (final `SECTIONS = [INTRO, SETUP, SDK_INIT, SCRATCH, TRIAGE, PROMOTION, PIPELINE, BRIEFING, RECALL, CONTINUITY, CLOSING]`)
- Create (generated): `notebooks/02_scheduled_briefings_todo.ipynb`

**Interfaces:**
- Consumes: everything prior (`write_note`, `drain_queue`, `promote.rerank_by_recency`, `latest_briefing`, sidecar table, `SUPERSEDE_*` paths).
- Produces: `recall(query, asset_id, k=5) -> list[dict]`; the supersession + continuity demos; closing verify. No new TODOs (total stays 5).

- [ ] **Step 1: Append to `tools/make_02_notebook.py`** (update `SECTIONS`)
...
- [ ] **Step 2: Regenerate, validate, produce the learner flavor**
...
.venv/bin/python - <<'EOF'
import json
todo = json.dumps(json.load(open('notebooks/02_scheduled_briefings_todo.ipynb')))
assert 'TODO-SOLUTION' not in todo
assert todo.count('your code here') == 5
print('learner notebook ok')
EOF
.venv/bin/pytest -q
```
Expected: `compiled 19 code cells; 5 TODO blocks present` (report actual count if arithmetic differs — proceed if all compile and TODO count is 5); `learner notebook ok`; 58 passed. Note: `make_student.py` will also regenerate `01_..._todo.ipynb` — that's expected and harmless; commit it only if it changed.

- [ ] **Step 3: Commit**

```bash
git add tools/make_02_notebook.py notebooks/02_scheduled_briefings_complete.ipynb notebooks/02_scheduled_briefings_todo.ipynb
git commit -m "feat: notebook 02 - supersession recall, continuity, closing; learner flavor
...
**Files:**
- Modify (only if the live run exposes bugs): `tools/make_02_notebook.py` + regenerate both flavors
- Modify: `README.md` (series row for 02)

**Interfaces:**
- Consumes: populated `.env`; `CREATE PROCEDURE` + `CREATE JOB` granted to `ANANT` (verify first — see Step 0).

- [ ] **Step 0: Confirm privileges**
...
with conn.cursor() as cur:
    cur.execute("SELECT privilege FROM user_sys_privs WHERE privilege IN ('CREATE PROCEDURE','CREATE JOB')")
    print(sorted(r[0] for r in cur.fetchall()))
EOF
```
Expected: `['CREATE JOB', 'CREATE PROCEDURE']`. If not, STOP and report — the user must run the grants as ADMIN (the exact statements are in the notebook's privilege-check cell); do not attempt to connect as ADMIN yourself.

- [ ] **Step 1: Reset phase-02 state from earlier attempts** (HARNESS_% only — never CITY_%)
...
conn = get_connection(load_settings())
with conn.cursor() as cur:
    for job in ("HARNESS_STAGE_JOB", "HARNESS_BRIEFING_JOB"):
        try:
            cur.callproc("DBMS_SCHEDULER.DROP_JOB", [job])
            print("dropped job", job)
        except Exception:
            print("no job", job)
    for proc in ("HARNESS_STAGE_INBOX", "HARNESS_BUILD_BRIEFING"):
        try:
            cur.execute(f"DROP PROCEDURE {proc}")
            print("dropped proc", proc)
        except Exception:
            print("no proc", proc)
    for t in ("HARNESS_SCRATCH", "HARNESS_PROMOTION_QUEUE", "HARNESS_MEMORY_META", "HARNESS_BRIEFING"):
        try:
...
```bash
.venv/bin/jupyter nbconvert --to notebook --execute notebooks/02_scheduled_briefings_complete.ipynb \
  --output /private/tmp/claude-501/-Users-anantsr-Documents-Projects-total-recall/11d74471-e897-46f3-8e21-bbfa42b4c813/scratchpad/02_executed.ipynb 2>&1 | tail -3
```
~10–20 LLM calls (triage per note, supersession checks, narration). If a cell fails: fix the generator, regenerate both flavors, reset per Step 1, re-run (max 3 total runs; then stop with DONE_WITH_CONCERNS and evidence). Do not weaken `check(...)` assertions.

- [ ] **Step 3: Inspect outputs** (same extraction snippet as phase 01, pointed at `02_executed.ipynb`)

Verify: SDK round-trip green; working files (`/work/`, `/tool_out/`) show `D` or `N`, never `P`/`S`; producer staged ≥5; queue health `ok` after drain; ≥3 promotions with sidecar rows; supersession check fires for the repair note; recall excludes the superseded defect note; briefing payload has real numbers and narration prints; continuity check green; final `state.verify` green.

- [ ] **Step 4: Update README series row for 02** to:
...
```bash
.venv/bin/pytest -q
git add README.md tools/make_02_notebook.py notebooks/
git commit -m "feat: notebook 02 verified live end-to-end
...
- Carry-overs to phase 03: Langfuse live score delivery still unverified (needs docker — Codespaces); jobs left scheduled on the ADB (documented in the notebook's closing); notebook 03 registers its artifacts in `state.REGISTRY`.
