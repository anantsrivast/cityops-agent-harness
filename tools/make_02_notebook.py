...
# --------------------------------------------------------------------------
# Section: promotion with provenance + supersession
# --------------------------------------------------------------------------
PROMOTION = [
    md(
    ...
    code(
        ...
        "    # Idempotent promotion: drop any earlier memory for THIS path first. Without\n"
        "    # this, re-running the notebook piles up duplicate copies of each note in the\n"
        "    # SDK store, and those duplicates crowd the real candidates out of the\n"
        "    # supersession search below - so supersession silently stops firing.\n"
        "    with conn.cursor() as cur:\n"
        "        cur.execute(\"SELECT memory_id FROM HARNESS_MEMORY_META WHERE source_path = :p\", p=path)\n"
        "        _prior = [r[0] for r in cur.fetchall()]\n"
        "    for _old in _prior:\n"
        "        try:\n"
        "            memory.delete_memory(_old)\n"
        "        except Exception:\n"
        "            pass\n"
        "    if _prior:\n"
        "        with conn.cursor() as cur:\n"
        "            cur.execute(\"DELETE FROM HARNESS_MEMORY_META WHERE source_path = :p\", p=path)\n"
        "        conn.commit()\n"
        ...
        "    #             max_results=5)), skip any already superseded (sidecar lookup),\n"
...
# --------------------------------------------------------------------------
# Section: the scheduled pipeline
# --------------------------------------------------------------------------
PIPELINE = [
    md(
...
# --------------------------------------------------------------------------
# Section: the briefing automation
# --------------------------------------------------------------------------
BRIEFING = [
    md(
...
# --------------------------------------------------------------------------
# Section: recall with provenance, recency, and supersession
# --------------------------------------------------------------------------
RECALL = [
    md(
    ...
    code(
        ...
        "# >= 1, not == 1: each promotion of the defect note adds a sidecar row, so re-running\n"
        "# the notebook (without a HARNESS_% reset) leaves several - all correctly superseded.\n"
        "check(_superseded >= 1, f\"defect memory marked superseded by the repair note \"\n"
        "                        f\"({_superseded} superseded sidecar row(s))\")"
    ),
    code(
        ...
        "# Assert by *superseded memory id*, not by path: a re-run may leave a fresh, live\n"
        "# defect memory (legitimately recalled) alongside the superseded one. The invariant\n"
        "# is that no memory the sidecar marks superseded ever surfaces.\n"
        "with conn.cursor() as cur:\n"
        "    cur.execute(\"SELECT memory_id FROM HARNESS_MEMORY_META WHERE superseded_by IS NOT NULL\")\n"
        "    _superseded_ids = {row[0] for row in cur.fetchall()}\n"
        "check(all(r[\"memory_id\"] not in _superseded_ids for r in results),\n"
        "      \"no superseded memory competes for retrieval attention\")"
    ),
]
