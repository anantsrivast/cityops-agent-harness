...
OFFLOAD = [
    ...
    code(
        "# Pick the asset with the most corrosion mentions - a real question with a real answer.\n"
        "with conn.cursor() as cur:\n"
        "    cur.execute(\n"
        "        \"SELECT asset_id, COUNT(*) FROM CITY_INSPECTION_FINDING\"\n"
        "        \" WHERE LOWER(DBMS_LOB.SUBSTR(description, 3000, 1)) LIKE '%corros%'\"\n"
        "        \" GROUP BY asset_id ORDER BY COUNT(*) DESC FETCH FIRST 1 ROWS ONLY\"\n"
        "    )\n"
        "    TARGET_ASSET, CORROSION_COUNT = cur.fetchone()\n"
        "\n"
        "with conn.cursor() as cur:\n"
        "    cur.execute(\n"
        ...
        "GROUND_TRUTH = sum(1 for r in EXPORT_ROWS if \"corros\" in (r[\"description\"] or \"\").lower())\n"
        ...
        "print(f\"ground truth: {GROUND_TRUTH} of {len(EXPORT_ROWS)} findings mention corrosion\")"
    ),
    code(
    ...
    code(
        ...
        "    \"Answer only from data you have actually seen. Give the number explicitly.\"\n"
        ")\n"
        "\n"
        "QUESTION = (f\"Export all inspection findings for {TARGET_ASSET}. How many of them\"\n"
        "            f\" mention corrosion, and which is the oldest?\")\n"
        "\n"
    ...
    code(
        ...
        "check(str(GROUND_TRUTH) in answer,\n"
        "      f\"the answer reports the correct count ({GROUND_TRUTH}) from data it went and got\")"
    ),
    md(
...
EXCLUSION = [
    ...
    code(
        ...
        "         f\"Season summary for {TARGET_ASSET}: {GROUND_TRUTH} corrosion findings.\"),\n"
        "    )\n"
        "conn.commit()\n"
        "\n"
        "with conn.cursor() as cur:\n"
